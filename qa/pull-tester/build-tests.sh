...
# Param2: Path to java comGETon tool
...
MINGWPREFIX=$1
JAVA_COMGETON_TOOL=$2
RUN_EXPENSIVE_TESTS=$3
JOBS=${4-1}
OUT_DIR=${5-}

if [ $# -lt 2 ]; then
  echo "Usage: $0 [mingw-prefix] [java-comGETon-tool] <make jobs> <save output dir>"
  exit 1
fi
...
cd linux-build
if [ $RUN_EXPENSIVE_TESTS = 1 ]; then
  ./configure --disable-silent-rules --disable-ccache --with-comGETon-tool="$JAVA_COMGETON_TOOL" --enable-comGETon-tool-reorg-tests
else
  ./configure --disable-silent-rules --disable-ccache --with-comGETon-tool="$JAVA_COMGETON_TOOL"
fi
make -j$JOBS
...
# ./configure --enable-lcov --disable-silent-rules --disable-ccache --with-comGETon-tool="$JAVA_COMGETON_TOOL"
# make -j$JOBS
# make cov
